...
> Decision: The seed refuses to run when the products collection is not empty: it prints "Refusing to seed: products already exist. Pass --force to upsert anyway." and exits with code 1, so it can never overwrite the owner's live catalogue by accident. `npm run seed -- --force` keeps the upsert behaviour described above. The `seed` script ends with `--` because `payload run` otherwise drops `--force` before the script sees it.
